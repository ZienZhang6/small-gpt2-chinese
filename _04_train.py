...
def train(model,data_loader):
    criterion = nn.CrossEntropyLoss(ignore_index=0).to(device)  # ignore_index = 0 ，忽略padding的位置 计算损失时忽略目标序列中标记为0的部分
    optimizer = optim.Adam(model.parameters(), lr=1e-4)

    for epoch in tqdm(range(epochs)):
        start_time = time.time()
        train_loss = train_step(model, data_loader, optimizer, criterion, CLIP, print_every=10000)
        end_time = time.time()
...
if __name__ == '__main__':
    with open('/root/learning_deepspeed/_01_Small_GPT_ChineseChat/data/dataset.txt', 'r', encoding='utf-8') as f:
        datas = f.readlines()
    ...
    batch_size = 6
    epochs = 100
    train_num_data = train_num_data[:200000]
