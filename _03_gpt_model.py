...
dict_datas = json.load(open('/root/learning_deepspeed/_01_Small_GPT_ChineseChat/data/dict_datas.json', 'r'))
word2id, id2word = dict_datas['word2id'], dict_datas['id2word']
vocab_size = len(word2id)
max_pos = 1800
d_model = 768  # Embedding Size
d_ff = 2048  # FeedForward dimension    # 前馈网络的维度，是经过多头注意力之后的输出的维度。
d_k = d_v = 64  # dimension of K(=Q), V # 多头注意力机制中键（K）和值（V）的维度。
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        context, attn = ScaledDotProductAttention()(Q, K, V, attn_mask) # 内积缩放，以更好地控制梯度的大小，避免梯度消失或爆炸问题
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs):
        ...
        # pad掩码：创建掩码矩阵，标记填充部分 (PAD tokens) 以避免在注意力机制中对其进行计算。
        dec_self_attn_pad_mask = get_attn_pad_mask(dec_inputs, dec_inputs) # [batch_size, tgt_len, tgt_len]

        # subsequence掩码：创建掩码矩阵，防止模型关注未来单词的信息 (避免泄露信息)。
        dec_self_attn_subsequence_mask = get_attn_subsequence_mask(dec_inputs)# [batch_size, tgt_len, tgt_len]

        # 将pad掩码 和 subsequence掩码结合起来
...
class GPT(nn.Module):
    def __init__(self):
        ...
        self.projection = nn.Linear(d_model,vocab_size) # projection 映射到词表大小的空间
...
# 调用print_num_parameters进行参数数量的统计
...
#     model.to(device)
#     summary(model, input_size=(1, 1280))